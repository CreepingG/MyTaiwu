...
namespace CharacterFloatInfo
{
    ...
    public static class WindowManage_WindowSwitch_Patch
    {
                ...
                if (Main.settings.showIV)
                {
                    if (DateFile.instance.HaveLifeDate(id, 901))
                    {
                        ...
                        if (rest > 0)
                        {
                            if (rest > 1000)
                            {
                                text += $"\t孕期剩余<color=white>{rest - 1000}</color>个月";
                            }
                            else
                            {
                                text += $"\t孕期剩余<color=white>{rest}</color>个月";
        ...
        //魅力
        public static string GetCharm(int id)
        {
    ...
    public static class WindowManage_ShowTips_Patch
    {
        public static void Postfix(WindowManage __instance, ref Vector3 ___tipsPoint, int ___tipsW, int ___tipsH)
        {
            ...
            if (__instance.tipsDate.CompareTag("PeopleActor"))
            {
                ...
                component.sizeDelta = new Vector2((float)___tipsW, (float)___tipsH + __instance.informationMassage.rectTransform.sizeDelta.y + __instance.informationName.rectTransform.sizeDelta.y);
